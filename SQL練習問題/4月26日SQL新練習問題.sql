...
select p.patient_name, d.doctor_name, dpm.department_name, dpm.description from patients p inner join examinations e on p.patient_id = e.patient_id left join doctors d on e.doctor_id = d.doctor_id left join departments dpm on e.department_id = dpm.department_id;

-- 45/ 患者の性別と年齢情報、医師の性別を結合するSQL文を作成してください。
select p.patient_name, p.gender, p.date_of_birth, extract(year from age(current_date, p.date_of_birth)) as age, d.doctor_name, d.gender from patients p inner join examinations e on p.patient_id = e.patient_id inner join doctors d on e.doctor_id = d.doctor_id;

-- 46/ 全患者の中で、診断された病気の数が最も多いトップ5の患者を取得するためのクエリを作成してください。
--ただし、診断がない患者は結果に含めないでください。
select p.patient_name ,count(e.patient_id) from patients p inner join examinations e on p.patient_id = e.patient_id group by p.patient_name limit 5;
